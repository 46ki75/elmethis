...
// anchor `.click()` semantics don't exist (or aren't enforced) in createDOM,
...
// confirmed against the react sibling spec with a throwaway harness before
// writing these; even a genuine Playwright-dispatched click can't get a real
// popup out of it. That rules out asserting on the opened window itself, but
// the blob: URL passed to `window.open` is still real and fetchable, so the
// wrapper markup it points to is inspected directly instead.
describe("[CSR] ElmHtmlViewer — open in new tab", () => {
  ...
  test("opens a blob: url in a new tab without noopener", async () => {
    ...
    // Each `$()` handler is its own QRL segment, lazily fetched on first
    // invocation — a real click resolves as soon as the DOM event fires, not
    // once that async fetch+execution completes, so the first click on a
    // given handler in a file needs a poll rather than a synchronous assert.
    await vi.waitFor(() => {
...
describe("[CSR] ElmHtmlViewer — download", () => {
  ...
  test("downloads with the default filename, via an anchor attached to the document", async () => {
    ...
    // handleDownload is a separate QRL segment from handleOpenInNewTab, so
    // this first click on it needs its own poll — see the "open in new tab"
