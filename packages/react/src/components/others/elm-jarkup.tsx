...
export const ElmJarkup = ({
  ...
  const render = (jsonComponents: Component[]): ReactNode[] => {
    return jsonComponents.map((component, index) => {
      ...
      switch (component.type) {
        ...
        case "Fragment": {
          return (
            <Fragment key={key}>{render(component.slots.default)}</Fragment>
          );
        }
        ...
        case "Toggle":
          return (
            <ElmToggle
              key={key}
              summary={render(component.slots.summary)}
              style={firstStyle}
            >
              {render(component.slots.default)}
            </ElmToggle>
        ...
        case "Tabs": {
          ...
          return (
            <ElmTabs key={key} defaultValue="0">
              <ElmTabList>
        ...
        case "ColumnList":
          return (
            <div key={key} className={styles["column-list"]} style={firstStyle}>
              {render(component.slots.default)}
            </div>
          );
        ...
        default:
          ...
          return (
            <ElmUnsupportedBlock
              key={key}
              details={`Unsupported component type: ${(component as Component).type}`}
              style={firstStyle}
            />
          );
      }
