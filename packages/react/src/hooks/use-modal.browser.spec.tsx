...
// is the half that the jsdom/happy-dom renderer cannot service — `showModal()`
// is not implemented there. That behavior lives in this real-browser spec.
...
describe("[CSR] useModal native dialog lifecycle", () => {
  test("is closed on mount — the dialog never entered the top layer", async () => {
    ...
    // Initially closed: the mount effect short-circuits, so showModal() is
    // never called and the content stays hidden.
  ...
  test("clicking the dialog content does not close it (stopPropagation guard)", async () => {
    ...
    // The content lives inside a `<div onClick={stopPropagation}>`, so a click
    // on the content must never reach the dialog's close handler.
