import type { Meta, StoryObj } from "@storybook/react-vite";
import type { CSSProperties } from "react";

import { useThrottledStore } from "./use-throttled-store";

// ---------------------------------------------------------------------------
// Shared helpers
// ---------------------------------------------------------------------------

const boxStyle: CSSProperties = { fontFamily: "monospace", padding: "1rem" };
const inputStyle: CSSProperties = {
  display: "block",
  marginBottom: "0.5rem",
  width: "100%",
};
const rowStyle: CSSProperties = {
  display: "flex",
  gap: "0.5rem",
  marginBottom: "0.5rem",
};
const preStyle: CSSProperties = {
  background: "#f4f4f4",
  padding: "0.5rem",
  margin: "0",
  fontSize: "0.85rem",
};

// ---------------------------------------------------------------------------
// Demo components
// ---------------------------------------------------------------------------

/**
 * Basic usage: a search form whose raw state is reflected in `value` on
 * every keystroke while `throttledValue` updates at most once every 500 ms
 * (leading edge). Ideal for live previews where you want to cap the rate
 * of downstream effects such as API calls.
 */
const BasicDemo = () => {
  const { value, setValue, throttledValue } = useThrottledStore(
    { query: "" },
    500,
  );

  return (
    <div style={boxStyle}>
      <p>
        Type quickly — <code>value</code> updates on every keystroke,{" "}
        <code>throttledValue</code> updates at most once every 500 ms.
      </p>
      <input
        style={inputStyle}
        placeholder="Search…"
        onInput={(e) => setValue({ query: e.currentTarget.value })}
      />
      <p>
        <strong>value (immediate):</strong>
      </p>
      <pre style={preStyle}>{JSON.stringify(value, null, 2)}</pre>
      <p>
        <strong>throttledValue (≤ 1 update / 500 ms):</strong>
      </p>
      <pre style={preStyle}>{JSON.stringify(throttledValue, null, 2)}</pre>
    </div>
  );
};

/**
 * Multi-field form: each field is patched independently. `throttledValue`
 * captures only the leading-edge patch per 600 ms window.
 */
const MultiFieldDemo = () => {
  const { value, setValue, throttledValue } = useThrottledStore(
    { first: "", last: "", email: "" },
    600,
  );

  return (
    <div style={boxStyle}>
      <p>
        Edit multiple fields — <code>throttledValue</code> captures at most one
        patch per 600 ms window.
      </p>
      <label style={{ display: "block", marginBottom: "0.25rem" }}>
        First name
        <input
          style={inputStyle}
          placeholder="First"
          onInput={(e) =>
            setValue((prev) => ({ ...prev, first: e.currentTarget.value }))
          }
        />
      </label>
      <label style={{ display: "block", marginBottom: "0.25rem" }}>
        Last name
        <input
          style={inputStyle}
          placeholder="Last"
          onInput={(e) =>
            setValue((prev) => ({ ...prev, last: e.currentTarget.value }))
          }
        />
      </label>
      <label style={{ display: "block", marginBottom: "0.5rem" }}>
        Email
        <input
          style={inputStyle}
          placeholder="Email"
          onInput={(e) =>
            setValue((prev) => ({ ...prev, email: e.currentTarget.value }))
          }
        />
      </label>
      <p>
        <strong>value (immediate):</strong>
      </p>
      <pre style={preStyle}>{JSON.stringify(value, null, 2)}</pre>
      <p>
        <strong>throttledValue (≤ 1 update / 600 ms):</strong>
      </p>
      <pre style={preStyle}>{JSON.stringify(throttledValue, null, 2)}</pre>
    </div>
  );
};

/**
 * Counter: click +1 rapidly — `throttledValue` only picks up the first
 * click in each 400 ms window.
 */
const CounterDemo = () => {
  const { value, setValue, throttledValue } = useThrottledStore(
    { count: 0 },
    400,
  );

  return (
    <div style={boxStyle}>
      <p>
        Click +1 rapidly — <code>throttledValue</code> only picks up the first
        click in each 400 ms window.
      </p>
      <div style={rowStyle}>
        <button onClick={() => setValue((prev) => ({ count: prev.count + 1 }))}>
          +1
        </button>
        <button onClick={() => setValue({ count: 0 })}>Reset</button>
      </div>
      <p>
        <strong>value (immediate):</strong>
      </p>
      <pre style={preStyle}>{JSON.stringify(value, null, 2)}</pre>
      <p>
        <strong>throttledValue (≤ 1 update / 400 ms):</strong>
      </p>
      <pre style={preStyle}>{JSON.stringify(throttledValue, null, 2)}</pre>
    </div>
  );
};

/**
 * Zero interval: when `interval` is 0, `throttledValue` is updated
 * synchronously — same as `value`.
 */
const ZeroIntervalDemo = () => {
  const { value, setValue, throttledValue } = useThrottledStore(
    { query: "" },
    0,
  );

  return (
    <div style={boxStyle}>
      <p>
        With <code>interval=0</code>, <code>throttledValue</code> updates
        synchronously — same as <code>value</code>.
      </p>
      <input
        style={inputStyle}
        placeholder="Type here…"
        onInput={(e) => setValue({ query: e.currentTarget.value })}
      />
      <p>
        <strong>value:</strong>
      </p>
      <pre style={preStyle}>{JSON.stringify(value, null, 2)}</pre>
      <p>
        <strong>throttledValue:</strong>
      </p>
      <pre style={preStyle}>{JSON.stringify(throttledValue, null, 2)}</pre>
    </div>
  );
};

// ---------------------------------------------------------------------------
// Meta
// ---------------------------------------------------------------------------

const meta: Meta = {
  title: "Hooks/useThrottledStore",
  tags: ["autodocs"],
};

export default meta;
type Story = StoryObj;

export const Basic: Story = {
  render: () => <BasicDemo />,
};

export const MultiField: Story = {
  render: () => <MultiFieldDemo />,
};

export const Counter: Story = {
  render: () => <CounterDemo />,
};

export const ZeroInterval: Story = {
  render: () => <ZeroIntervalDemo />,
};
