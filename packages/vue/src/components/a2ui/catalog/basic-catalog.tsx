 ...
 *
 * Each entry is a typed `defineRenderer(api, fn)`. Interactive components write
...
export const basicCatalog: CatalogRenderer = new CatalogRenderer([
  ...
  // render <div>s. <div> inside <button> is invalid HTML.
      ...
      <div class={styles["text-field"]}>
        ...
        {variant === "longText" ? (
          <textarea
            ...
            value={value}
            onInput={(e: Event) =>
              setBinding("value", (e.target as HTMLTextAreaElement).value)
            }
            onChange={(e: Event) =>
              setBinding("value", (e.target as HTMLTextAreaElement).value)
            }
    ...
    return (
      <label class={styles.checkbox}>
        <input
          ...
          checked={checked}
          onChange={(e: Event) =>
            setBinding("value", (e.target as HTMLInputElement).checked)
          }
        />
  ...
  defineRenderer(SliderApi, ({ props, ctx, setBinding }) => {
    ...
    return (
      <input
        ...
        max={props.max}
        onInput={(e: Event) =>
          setBinding("value", Number((e.target as HTMLInputElement).value))
        }
        onChange={(e: Event) =>
          setBinding("value", Number((e.target as HTMLInputElement).value))
        }
      ...
      return (
        <div
          ...
          onChange={(e: Event) => {
            ...
            // For single-select radios the browser keeps exactly one input
            // checked, so `checked` is already the right shape.
            setBinding("value", isMulti ? checked : checked.slice(0, 1));
          }}
        >
          {props.label ? (
