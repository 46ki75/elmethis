 ...
 * A2UI data layer (`@a2ui/web_core`) and the Vue view layer: an immutable
...
/**
 * Everything a render function needs to produce a VNode for a single component
 * instance. The helpers (`resolve`, `childRefs`, `renderChild`, `setBinding`,
...
export interface RenderArgs<TProps = Record<string, unknown>> {
   ...
   * no-op. The surface and component id are baked in by `ComponentHost`.
   */
  setBinding: (propName: string, value: unknown) => void;
   ...
   */
  dispatchAction: (propName?: string) => void;
}
