import { defineComponent, type HTMLAttributes } from "vue";
import { clsx } from "clsx";

import { ElmInlineText } from "../typography/elm-inline-text";
import styles from "./elm-unsupported-block.module.css";

export interface ElmUnsupportedBlockProps extends HTMLAttributes {
  details?: string;
}

export const ElmUnsupportedBlock = defineComponent({
  name: "ElmUnsupportedBlock",
  inheritAttrs: false,
  props: {
    details: { type: String, default: undefined },
  },
  setup(props, { attrs }) {
    return () => {
      const { class: className, ...rest } = attrs as Record<string, unknown>;

      return (
        <div
          class={clsx(
            styles["elm-unsupported-block"],
            className as string | undefined,
          )}
          {...rest}
        >
          <div class={styles.message}>
            <svg
              viewBox="0 0 24 24"
              width="1.25rem"
              height="1.25rem"
              class={styles.icon}
            >
              <path d="M13,9H11V7H13M13,17H11V11H13M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2Z" />
            </svg>
            <ElmInlineText>UNSUPPORTED BLOCK</ElmInlineText>
          </div>
          {props.details && (
            <div class={styles.details}>
              <ElmInlineText>{props.details}</ElmInlineText>
            </div>
          )}
        </div>
      );
    };
  },
});
