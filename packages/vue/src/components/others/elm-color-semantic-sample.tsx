...
export type ElmColorSemanticSampleProps = HTMLAttributes;
...
export const ElmColorSemanticSample = defineComponent({
  ...
  setup() {
    ...
    const renderColorSample = (variables: string[]): VNodeChild => (
    ...
    const renderPanel = (theme: "light" | "dark"): VNodeChild => (
      // `color-scheme` drives native light-dark() token resolution; `data-theme`
      // covers the few non-color overrides that can't use it. Plain elements
      // (not <html>/<body>) so the sample can be nested inside a page.
      <div
        class={styles.panel}
        data-theme={theme}
        ...
        >
          ...
          <div class={styles.body}>
            ...
            <div class={styles.group}>
              ...
              {renderColorSample(NEUTRAL_TOKENS)}
            </div>

            <div class={styles.group}>
              <span class={styles["section-title"]}>Primary</span>
              {renderColorSample(PRIMARY_TOKENS)}
            </div>
    ...
    // inheritAttrs default: a passthrough class merges with ours.
    return () => (
      <div class={clsx(styles["elm-color-semantic-sample"])}>
