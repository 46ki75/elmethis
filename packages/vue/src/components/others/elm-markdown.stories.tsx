...
const StreamingWrapper = defineComponent({
  name: "StreamingWrapper",
  setup() {
    const content = ref("");
    let aborted = false;
...
export const Stream: Story = {
  ...
  args: { markdown: "" },
  render: () => ({
    components: { StreamingWrapper },
    template: "<StreamingWrapper />",
  }),
};
...
export const BlogEnglish: Story = {
  args: { markdown: enMd },
  render: (args) => ({
    components: { ElmMarkdown },
    setup() {
      return { args };
    },
    template:
      '<div style="width: 540px; margin: 0 auto;"><ElmMarkdown v-bind="args" /></div>',
  }),
};
