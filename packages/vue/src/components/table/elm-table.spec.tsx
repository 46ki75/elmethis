...
import { createSSRApp, defineComponent, type VNode } from "vue";
...
// Mount a JSX tree (so the provide/inject chain is wired) and return its HTML,
// lowercased so callers can match `<th>` / `<td>` / attributes without casing.
const renderHTML = (factory: () => VNode) =>
  mount(defineComponent({ render: factory }))
    .html()
    .toLowerCase();
  ...
  test("native td attrs (colspan) flow through to <th>", () => {
    const html = renderHTML(() => (
      <ElmTable>
        <ElmTableHeader>
          <ElmTableRow>
            {/* colspan is a native fallthrough attr (not a declared prop). */}
...
// An <ElmTableCell> with no surrounding <ElmTable> / section providers should
// still render — the cell falls back to the inject defaults.
...
describe("[SSR] ElmTable", () => {
  test("renders the table shell with caption server-side", async () => {
    const html = (
